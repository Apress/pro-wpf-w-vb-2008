	...
	Public Function GetProduct(ByVal ID As Integer) As Product
		...
		Try
			...
			If reader.Read() Then
				...
				' current record.
				Dim product As New Product(CStr(reader("ModelNumber")), CStr(reader("ModelName")), CDec(reader("UnitCost")), CStr(reader("Description")), CStr(reader("ProductImage")))
				Return product
			Else
				Return Nothing
			End If
	...
	Public Function GetProducts() As ICollection(Of Product)
		...
		Try
			...
			Do While reader.Read()
				' Create a Product object that wraps the 
				' current record.
				Dim product As New Product(CStr(reader("ModelNumber")), CStr(reader("ModelName")), CDec(reader("UnitCost")), CStr(reader("Description")), CStr(reader("CategoryName")), CStr(reader("ProductImage")))
				' Add to collection
